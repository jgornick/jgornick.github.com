  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/debug') {
      ...
      try {
        ...
        const repoResp = await fetch('https://api.github.com/repos/jgornick/jgornick.github.com', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        const debugInfo = {
          user: {
            ...
            scopes: userResp.headers.get('X-OAuth-Scopes'),
            data: await userResp.json()
          },
          repo: {
            status: repoResp.status,
            data: await repoResp.json()
          }
        };
...
async function handleCallback(url, env, corsHeaders) {
  ...
  try {
    ...
    // Transform GitHub OAuth response to Decap CMS format
    const decapToken = {
    ...
    const tokenData = JSON.stringify(decapToken);
    const html = `
<!DOCTYPE html>
<html>
<head>
